      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-4 mb-3">
          ...
          {element.since && element.since.length > 0 && (
            <div className="flex items-center gap-1.5 px-2 py-1 bg-orange-50 text-orange-600 rounded-md text-sm font-medium border border-orange-100 shrink-0">
              <Clock size={14} />
              {element.since[0]}
            </div>
          )}
        ...
        <div className="space-y-4">
          ...
          {element.patterns && element.patterns.length > 0 && (
            <div>
              <span className="text-xs font-bold uppercase tracking-wider text-gray-400 flex items-center gap-1.5 mb-2">
               ...
               <div className="text-gray-600 text-sm leading-relaxed prose prose-sm max-w-none">
                  <ReactMarkdown 
                    ...
                    components={{
                      ...
                      code: ({node, ...props}) => <code className="bg-gray-100 px-1 rounded text-orange-600 font-mono text-xs" {...props} />,
                      strong: ({node, ...props}) => <strong className="font-bold text-gray-800" {...props} />,
                      a: ({node, ...props}) => <a className="text-orange-600 hover:text-orange-700 font-bold underline decoration-orange-200 underline-offset-2 transition-colors" {...props} />,
                    }}
                  >
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {element["event values"].map((val, i) => (
                  <span key={i} className="px-2 py-1 bg-blue-50 text-blue-600 rounded text-xs font-mono border border-blue-100">
                    {val}
                  </span>
                ))}
              </div>
          ...
          {element.usage && (
            <div>
              <span className="text-xs font-bold uppercase tracking-wider text-gray-400 block mb-1">Values</span>
              <div className="text-gray-600 text-xs bg-gray-50 p-2 rounded border border-gray-100 italic">
                {element.usage}
              </div>
            </div>
          )}
