...
import {BookOpen, ChevronRight, Menu, Moon, PanelLeftClose, PanelLeftOpen, Search, Settings, Sun} from "lucide-react";
        ...
        <div className="min-h-screen flex flex-col md:flex-row transition-all duration-300">
            ...
            <aside className={cn(
                ...
                <div className={cn("h-full flex flex-col transition-all duration-300", isCollapsed ? "p-3" : "p-5")}>
                    <div
                        className={cn("flex items-center gap-3 mb-8 overflow-hidden whitespace-nowrap", isCollapsed ? "justify-center mt-2" : "")}>
                        <div
                            className="flex-shrink-0 w-10 h-10 bg-gradient-to-br from-orange-400 via-orange-500 to-orange-600 rounded-xl flex items-center justify-center shadow-lg shadow-orange-500/20 dark:shadow-none ring-4 ring-orange-500/10">
                            <BookOpen className="text-white" size={22}/>
                        </div>
                        {!isCollapsed && (
                            <div className="transition-opacity duration-300">
                                <h1 className="text-lg font-black text-[var(--foreground)] tracking-tight">SkBee
                                    Docs</h1>
                                <div className="flex items-center gap-2">
                                    {docs &&
                                        <p className="text-[10px] font-black text-orange-500 uppercase tracking-tighter bg-orange-500/10 px-1.5 rounded">v{docs.metadata.version}</p>}
            ...
            <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden bg-[var(--background)]">
                {/* Header */}
                <header
                    className="bg-[var(--header-bg)] glass border-b border-[var(--border-color)] px-4 md:px-8 py-5 sticky top-0 z-30 transition-all duration-300">
                ...
                <div ref={scrollContainerRef} className="flex-1 overflow-y-auto p-4 md:p-10 custom-scrollbar">
                    <div className="max-w-5xl mx-auto">
                        {!docs ? (
                            ...
                            >
                                <div
                                    className="w-32 h-32 bg-gradient-to-br from-orange-400/20 to-orange-600/20 rounded-[40px] flex items-center justify-center relative">
                                ...
                                <div>
                                    <h2 className="text-3xl font-black text-[var(--foreground)] tracking-tight">No
                                        Documentation Found</h2>
                                    <p className="text-[var(--text-muted)] max-w-sm mx-auto mt-3 font-medium leading-relaxed">
                                        Ready to set up your docs? Login to the Admin Panel and upload your <code
                                        className="text-orange-600 dark:text-orange-400 bg-orange-500/10 px-2 py-0.5 rounded-lg font-bold">json-docs.json</code> file.
                                    </p>
                                </div>
                                <button
                                    onClick={() => setIsAdminOpen(true)}
                            ...
                            <div className="space-y-10">
                                <div className="flex items-end justify-between px-2">
                                    <div>
                                        ...
                                        <p className="text-[var(--text-muted)] mt-1 font-bold text-sm opacity-60 uppercase tracking-widest">{filteredElements.length} elements
                                            available</p>
                                    </div>
                                </div>
                                ...
                                <AnimatePresence mode="wait">
                                    ...
                                    >
                                        {filteredElements.length > 0 ? (
                                            ...
                                            <div
                                                className="bg-[var(--card-bg)] rounded-[32px] p-20 text-center border-2 border-dashed border-[var(--border-color)]">
                                                <div
                                                    className="w-20 h-20 bg-orange-500/5 rounded-full flex items-center justify-center mx-auto mb-6">
                                                    <Search className="text-orange-500/30" size={40}/>
                                                </div>
                                                <p className="text-[var(--text-muted)] font-black text-xl tracking-tight">Zero
                                                    matches found</p>
                                                <p className="text-[var(--text-muted)] opacity-60 mt-2 font-medium">Try
                                                    adjusting your search terms or category.</p>
                                            </div>
                                        )}
                                    </motion.div>
                                </AnimatePresence>
